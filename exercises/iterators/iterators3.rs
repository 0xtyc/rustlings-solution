...
mod tests {
    ...
    fn test_result_with_list() {
        assert_eq!(format!("{:?}", result_with_list(
            vec![27, 297, 38502, 81]
        )), "Ok([1, 11, 1426, 3])");
    }
    ...
    fn test_result_with_list_error() {
        assert_eq!(format!("{:?}", result_with_list(
            vec![27, 297, 38502, 80]
        )), "Err(NotDivisible(NotDivisibleError { dividend: 80, divisor: 27 }))");
    }
    ...
    #[test]
    fn test_list_of_results_error() {
        assert_eq!(
            format!("{:?}", list_of_results(vec![27, 297, 38502, 80])),
            "[Ok(1), Ok(11), Ok(1426), Err(NotDivisible(NotDivisibleError { dividend: 80, divisor: 27 }))]"
        );
    }
}
